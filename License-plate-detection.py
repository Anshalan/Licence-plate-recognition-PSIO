...
# # Testowy
# edged = cv.Canny(gray, 50,230)
# cv.imshow("3 - Canny",edged)
# cv.waitKey(0)

# Canny do VW Touran 2m
# edged = cv.Canny(gray, 230,255)
# cv.imshow("3 - Canny",edged)
# cv.waitKey(0)
...
for c in cnts:
    ...
    # print ("krok" + str(count))
    if len(approx) == 4:
        NumberPlatCnt = approx
        x, y, w, h, =cv.boundingRect(c)
        diagonalpow2 = pow(w,2)+pow(h,2)

        if w > 165 and h > 36 and w > (3 * h) and w < (5 * h) and diagonalpow2 < (pow(h, 2) + pow(w, 2) + 20) and diagonalpow2 > (pow(h, 2) + pow(w, 2) - 20): #Jednorzędowe tablice dla samochodów oraz przyczep	520 x 114 mm =>4:1
...
# TouranFront3
# upperColor = (255, 120, 60)
# lowerColor = (110, 30, 10)

# CorsaRed1
# upperColor = (255, 100, 20)
# lowerColor = (110, 40, 10)

# Touran Angle
# upperColor = (255, 120, 60)
# lowerColor = (110, 50, 10)

# BMWFront1 + BMWFront2
# upperColor = (255, 60, 20)
# lowerColor = (110, 40, 10)
